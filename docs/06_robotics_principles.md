...
# 06. Kinematics, inverse kinematics, Programming of a simulated robotic arm
...
3. Compute $\Delta\mathbf{ q}=\mathbf{J}^{-1}(\mathbf{q_0})\cdot \left[\matrix{k_1 \cdot \Delta\mathbf{r} \\ k_2 \cdot \Delta\phi\mathbf{t}}\right]$, where the inverse can be pseudo-inverse or transposed
